...
THEATER_SIZES = {"Kino": 10, "Regal": 16, "Skandia": 100}
...
def check_ping():
    ...
    try:
        ...
        abort_on_resource(
            "GET", resource, "does not return pong -- is your server running?"
        )
...
def check_get_movies_with_query(title, year):
    ...
    try:
        ...
        movies = response_to_dicts(r)
        expected = set(
            imdb_key for t, imdb_key, y in MOVIES if t == title and y == year
        )
        found = set(movie["imdbKey"] for movie in movies)
        require(found, expected)
        ok("GET", resource)
...
def check_get_performances(actual_performances):
    ...
    try:
        ...
        found = set(
            performance["performanceId"] for performance in returned_performances
        )
...
def check_ticket_hoarding():
    ...
    for username, counts in users.items():
        ...
        for perf_info in summary:
            ...
            require(
                perf_info["nbrOfTickets"],
                users[username][perf_id_lookup[(theater, date, start_time)]],
